...
class RobotOperationManager { // 시퀀스 다이어그램에서 볼 수 있듯이 이 클래스가 모든거 다 지시
   ...
   RobotOperationManager() throws IOException {
      ...
      sim.TranslateMoveOperation(map.current_x, map.current_y, cur_direction); //나중에 이거 함수 바꿔야한다.
      //sim에게 현재 상태를 입력해줌
      
      do {//일단 한번은 무조건 path생성
    	    cbsm.AddSensorValue(sim, map.current_x, map.current_y, cur_direction);
            

            if (hsm.AddSensorValue(sim, map.current_x, map.current_y, cur_direction)) { //Hazard point 발견 시
            ...
            System.out.println(next_opr + " , " + operation_list[next_opr]);
            	...
            	int tempx = map.current_x;
            	int tempy = map.current_y;
            	SetCurrentPosition(SIM.current_x , SIM.current_y);
            	if (operation_list[next_opr].equals("MOVE")) {
                    if (psm.AddSensorValue(sim, map.current_x, map.current_y, cur_direction)) {
...
abstract class SensorManager{ // 이거도 손봐야함.
   abstract boolean AddSensorValue(SIM sim, int x, int y, int dir);
...
class HazardSensorManager extends SensorManager{
   ...
   boolean AddSensorValue(SIM sim, int x, int y, int dir){
	  return sim.HazardSensor(x, y, dir);
...
class ColorBlobSensorManager extends SensorManager{
   boolean AddSensorValue(SIM sim, int x, int y, int dir) {
	  return sim.ColorBlobSensor(x, y, dir);
...
class PositionSensorManager extends SensorManager{
   boolean AddSensorValue(SIM sim, int x, int y, int dir) {
	  return sim.CurrentPositionSensor(x, y, dir);
...
class SIM { //SIM 클래스. 
   ...
   boolean ColorBlobSensor(int x, int y, int dir) {
	   int[] dx = {0, -1, 0, 1};
	      int[] dy = {-1, 0, 1, 0};
	      boolean check = false;
	      for(int i=0; i<4; i++) {
   ...
   boolean HazardSensor(int x, int y, int dir) {
	      ...
	      if(x+dx[dir] >= 0 && y+dy[dir] >= 0 && x+dx[dir] < Map.mapsize_row && y+dy[dir] < Map.mapsize_col) {
	         if(Map.map[x+dx[dir]][y+dy[dir]] == 5) {
	        	...
	        	MapDataManager.GUIMapSetPrint("SEENHAZARD", x+dx[dir], y+dy[dir]);
   ...
   boolean CurrentPositionSensor(int x, int y, int dir) {
